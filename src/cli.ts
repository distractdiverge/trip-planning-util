...
import { PlanInputSchema } from './lib/types.js';
  ...
  .action(async (options) => {
    try {
      ...
      if (options.arrive.includes('T') || options.arrive.includes('Z')) {
        ...
        const dateTime = DateTime.fromFormat(`${options.date} ${options.arrive}`, 'yyyy-MM-dd h:mm a');
        eventTime = dateTime.toISO()!;
      }

      // Resolve addresses from aliases
      const resolveAddress = (name?: string, address?: string) => {
